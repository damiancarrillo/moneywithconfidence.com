<template>
  <section class="bg-pink-50 py-14">
    <div class="mx-auto max-w-6xl px-6 lg:px-8">
      <div
        class="mx-auto grid max-w-2xl grid-cols-1 lg:mx-0 lg:max-w-none lg:grid-cols-2"
      >
        <div class="flex flex-col pb-10 sm:pb-16 lg:pb-0 lg:pr-8 xl:pr-20">
          <figure class="mt-2 flex flex-auto flex-col justify-between">
            <blockquote class="text-lg leading-8 text-pink-900">
              <p>
                &ldquo;The most important thing I learned today was
                understanding the steps we should take toward building wealth. I
                got the house, I saved the money…my question was now what? I
                leave today with more insight and I appreciate you for
                it.&rdquo;
              </p>
            </blockquote>
            <figcaption class="mt-10 flex items-center gap-x-6">
              <img
                class="h-14 w-14 rounded-full bg-gray-50"
                src="/pexels-thisisengineering-3862601.webp"
                alt=""
              />
              <div class="text-base">
                <div class="font-semibold text-pink-950">
                  Workshop Participant
                </div>
                <div class="mt-1 text-pink-700">Breakthrough Austin</div>
              </div>
            </figcaption>
          </figure>
        </div>
        <div
          class="flex flex-col border-t border-red-900/10 pt-10 sm:pt-16 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0 xl:pl-20"
        >
          <figure class="mt-10 flex flex-auto flex-col justify-between">
            <blockquote class="text-lg leading-8 text-pink-900">
              <p>
                &ldquo;Natasha is very knowledgeable with great ideas for how to
                maximize your money that&rsquo;s suitable for your lifestyle.
                She understands finances aren&rsquo;t just a one size fits all
                approach.&rdquo;
              </p>
            </blockquote>
            <figcaption class="mt-10 flex items-center gap-x-6">
              <img
                class="h-14 w-14 rounded-full bg-gray-50"
                src="/pexels-pixabay-157776.webp"
                alt=""
              />
              <div class="text-base">
                <div class="font-semibold text-red-950">Andrea F.</div>
                <div class="mt-1 text-pink-700">Clarity Session Client</div>
              </div>
            </figcaption>
          </figure>
        </div>
      </div>
    </div>
  </section>
</template>
