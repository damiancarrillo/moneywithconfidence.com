<template>
  <div class="pt-16 pb-10 bg-white lg:pt-24">
    <div class="px-6 mx-auto max-w-7xl lg:px-8">
      <div
        class="grid items-start max-w-2xl grid-cols-1 mx-auto gap-x-8 gap-y-16 sm:gap-y-24 lg:mx-0 lg:max-w-none lg:grid-cols-2"
      >
        <div>
          <div class="text-base leading-7 text-gray-900 lg:max-w-lg">
            <p class="text-base font-semibold leading-7 text-sky-500">
              So let's do this
            </p>
            <h1
              class="mt-2 text-3xl font-bold tracking-tight text-gray-900 font-header sm:text-4xl"
            >
              I can't wait to meet you
            </h1>
            <div class="max-w-xl">
              <p class="mt-6 text-lg">
                My money coaching and workshop sessions are designed with you in
                mind. I care about your growth and your forward progress, and
                that is my top priority.
              </p>
              <p class="mt-8">
                I have a BS in early childhood through 4th grade education and a
                Master&rsquo;s degree in Educational Technology. I worked in the
                education field as an elementary teacher, K-5 instructional
                coach, and childcare provider for 10 years. I hold a K-12
                principal&rsquo;s certification and completed a certification to
                be a Financial Education Instructor. I currently volunteer with
                <a
                  class="text-purple-800 underline hover:text-purple-400"
                  href="https://3rddecade.org/"
                  >3rd Decade</a
                >
                as a financial mentor.
              </p>
              <p class="mt-8">
                This background gives me an excellent foundation to be your
                financial coach and educator. I have a deep passion for money
                topics and an uncanny ability to see beyond the numbers.
                <span class="font-semibold"
                  >I can&rsquo;t wait to help you find your clear path
                  forward</span
                >.
              </p>
            </div>
            <div class="flex mt-10">
              <a
                href="/services"
                class="px-5 py-3 text-base font-semibold text-white rounded-full shadow-lg bg-sky-500 hover:bg-sky-400 focus-visible:outline-sky-600 shadow-sky-500/50 hover:shadow-sky-400/30"
                >Let&rsquo;s work together</a
              >
            </div>
          </div>
        </div>
        <div class="lg:pr-4">
          <img
            class="object-cover object-top shadow-xl aspect-square rounded-xl"
            src="/natasha-computer.webp"
            alt="Natasha drinking coffee while at the computer"
          />
        </div>
      </div>
    </div>
  </div>
</template>
